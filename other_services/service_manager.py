...
async def on_message_validation(validation_client, topic, payload, qos, properties,validation_incomming_calls_queue):
    #put the data in the calls queue for worker to process
    service_data={"topic":topic,
          "data":payload.decode('utf-8')}
    service_data=json.dumps(service_data)
    await validation_incomming_calls_queue.put(service_data)
...
async def respond_msg(bridge_client,topic:str,msg:str):
    ...
    if topic==subscribe_topics['bridge_call_recv_topic']:
        ...
        if service_data['condition']=="UNDERLOAD":
            ...
            await get_bridge_services_status(bridge_client=bridge_client,call_to_bridge_service_topic=publish_topics['bridge_call_topic'])
        ...
        pass
    #bridge_client.publish(topic,msg)

async def worker(bridge_client,queue):# checks the queue for incomming calls and sends them to be processed and maintains the set of active bridge services
    while True:
        ...
        if collect_bridge_status_reponse:
            try:
                ...
                pass
        await respond_msg(bridge_client,msg['topic'],msg['data'])        # slow work here
        queue.task_done()

def on_disconnect(bridge_client, packet, exc=None):# just prints diconnected used to see when disconnected
    print("Disconnected ...")
    print()

def on_subscribe(bridge_client, mid, qos, properties):# just prints subscribed used to see when disconnected
...
async def main():
    ...
    bridge_client_id=str(uuid.uuid4())
    
    bridge_client=mqtt(bridge_client_id)
    ...
    await bridge_client.connect(host=host,
                         port=port,
                         ssl=ssl_ctx)
    ...
    validation_client=mqtt(validation_client_id)
    ...
    validation_client.on_disconnect = on_disconnect_validation
    validation_client.on_subscribe=on_subscribe_validation


    asyncio.create_task(worker(bridge_client=bridge_client,queue=incomming_calls_queue))
